...
public extension GNode {
  ...
  subscript<E>(dynamicMember kp: ReferenceWritableKeyPath<T, E>) -> (GState<E.RawValue>) -> Self where E: RawRepresentable {
    { state in
      var s = self
      s.ops.append { node in
        state.observe(owner: node) { raw in
          guard let e = E(rawValue: raw) else {
            GD.printErr("Invalid rawValue for \(E.self):", raw)
            return
          }
          node[keyPath: kp] = e
        }
...
public extension GNode {
  /// Bind a GState to a keyPath, updating the node property whenever it changes
  /// Usage: .bind(\.position, to: $position)
  func bind<V>(_ kp: ReferenceWritableKeyPath<T, V>, to source: GState<V>) -> Self {
    var s = self
    s.ops.append { node in
      source.observe(owner: node) { value in
        node[keyPath: kp] = value
      }
    }
    return s
  ...
  func bind<V, U>(_ kp: ReferenceWritableKeyPath<T, U>, to source: GState<V>, transform: @escaping (V) -> U) -> Self {
    var s = self
    s.ops.append { node in
      source.observe(owner: node) { value in
        node[keyPath: kp] = transform(value)
  ...
  func bind<V, U>(_ kp: ReferenceWritableKeyPath<T, U>, to source: GState<V>, _ sourceKeyPath: KeyPath<V, U>) -> Self {
    var s = self
    s.ops.append { node in
      source.observe(owner: node) { value in
  ...
  /// Update node when GState changes using a custom closure
  /// Usage: .watch($myState) { node, value in ... }
  func watch<V>(_ source: GState<V>, _ handler: @escaping (T, V) -> Void) -> Self {
    var s = self
    s.ops.append { node in
      source.observe(owner: node) { value in
        handler(node, value)
...
public extension GNode {
  ...
  func bind<V1, V2, U>(
    ...
    s.ops.append { node in
      ...
      state1.observe(owner: node) { _ in update() }
      state2.observe(owner: node) { _ in update() }
    }
    return s
  }
  ...
  func bind<V1, V2, V3, U>(
    ...
    s.ops.append { node in
      ...
      state1.observe(owner: node) { _ in update() }
      state2.observe(owner: node) { _ in update() }
      state3.observe(owner: node) { _ in update() }
    }
    return s
  }
  ...
  func bind<V1, V2, V3, V4, U>(
    ...
    s.ops.append { node in
      ...
      state2.observe(owner: node) { _ in update() }
      state3.observe(owner: node) { _ in update() }
      state4.observe(owner: node) { _ in update() }
    }
    return s
  }
  ...
  func bind<V1, V2, V3, V4, V5, U>(
    ...
    s.ops.append { node in
      ...
      state3.observe(owner: node) { _ in update() }
      state4.observe(owner: node) { _ in update() }
      state5.observe(owner: node) { _ in update() }
    }
    return s
  }
  ...
  func bind<V1, V2, V3, V4, V5, V6, U>(
    ...
    s.ops.append { node in
      ...
      state4.observe(owner: node) { _ in update() }
      state5.observe(owner: node) { _ in update() }
      state6.observe(owner: node) { _ in update() }
    }
    return s
  }
}
