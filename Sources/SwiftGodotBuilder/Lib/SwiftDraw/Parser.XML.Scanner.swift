...
public extension XMLParser {
    struct Scanner {
        ...
        public init(text: String) {
            scanner = Foundation.Scanner(string: text)
            currentIndex = scanner.currentIndex
            scanner.charactersToBeSkipped = Foundation.CharacterSet.whitespacesAndNewlines
        }

        public var isEOF: Bool { return scanner.isAtEnd }

        @discardableResult
        public mutating func scanString(_ token: String) throws -> Bool {
            return try scanString(matchingAny: [token]) == token
        }

        @discardableResult
        public mutating func scanStringIfPossible(_ token: String) -> Bool {
            return (try? scanString(token)) == true
        }

        @discardableResult
        public mutating func nextScanString(_ token: String) -> Bool {
            scanner.currentIndex = currentIndex
            defer { scanner.currentIndex = currentIndex }
            return scanStringIfPossible(token)
        }

        public mutating func scanString(matchingAny tokens: Set<String>) throws -> String {
            scanner.currentIndex = currentIndex
        ...
        public mutating func scanCase<T: RawRepresentable & CaseIterable>(from type: T.Type) throws -> T where T.RawValue == String {
            scanner.currentIndex = currentIndex
        ...
        public mutating func scanString(matchingAny characters: Foundation.CharacterSet) throws -> String {
            scanner.currentIndex = currentIndex
            guard
        ...
        public mutating func doScanString(_ string: String) -> Bool {
            scanner.currentIndex = currentIndex
        ...
        public mutating func scanString(upTo token: String) throws -> String {
            scanner.currentIndex = currentIndex
        ...
        public mutating func scanString(upTo characters: Foundation.CharacterSet) throws -> String {
        ...
        public mutating func scanCharacter(matchingAny characters: Foundation.CharacterSet) throws -> Character {
            let location = currentIndex
        ...
        public mutating func scanUInt8() throws -> UInt8 {
            scanner.currentIndex = currentIndex
            var longVal: UInt64 = 0
            guard
                scanner.scanUnsignedLongLong(&longVal),
                let val = UInt8(exactly: longVal)
            else {
                throw Error.invalid
            }
            currentIndex = scanner.currentIndex
        ...
        public mutating func scanFloat() throws -> Float {
            scanner.currentIndex = currentIndex
        ...
        public mutating func scanUnit(_ unit: DOM.Unit) -> Bool {
            scanner.currentIndex = currentIndex
        ...
        public mutating func scanUnit() -> DOM.Unit? {
            if scanUnit(.pixel) {
                return .pixel
        ...
        public mutating func scanLength() throws -> DOM.Length {
            scanner.currentIndex = currentIndex
            guard
        ...
        public mutating func scanBool() throws -> Bool {
            return try scanCase(from: Boolean.self).boolValue
        }

        public mutating func scanCoordinate() throws -> DOM.Coordinate {
            let double = try scanDouble()
            let unit = scanUnit() ?? .pixel
            return DOM.Coordinate(double.apply(unit: unit))
        }

        public mutating func scanPercentageFloat() throws -> Float {
            scanner.currentIndex = currentIndex
        ...
        public mutating func scanAlpha() throws -> Float {
        ...
        public mutating func scanPercentage() throws -> Float {
