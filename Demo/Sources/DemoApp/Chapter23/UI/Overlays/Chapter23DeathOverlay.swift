...
extension Chapter23 {
  struct DeathOverlay: GView {
    ...
    var livesRemainingText: String {
      let lives = vm.playerLives
      if lives == 1 {
        return "LAST LIFE!"
      } else {
        ...
        CenterContainer$ {
          VBoxContainer$ {
            ...
            Label$()
              ...
              .theme(["fontSize": 32, "fontColor": palette.redLight])
              .ref($textNode)
            ...
            // Lives remaining
            Label$()
              .bind(\.text, to: state, \.playerLives) { lives in
                if lives == 1 {
                  return "LAST LIFE!"
                } else {
                  return "\(lives) LIVES LEFT"
    ...
    func playDeathAnimation() {
      ...
      Engine.onNextFrame { [router, vm] in
