    ...
    var body: some GView {
      ...
      return Node2D$ {
        ...
        .onSignal(\.areaEntered) { _, _ in
          // Colliding with player's attack hitbox (melee)
          if !isDying {
            let hitPos = position + [size / 2, size / 2]
    ...
    func takeDamage(_ damage: Int, from sourcePosition: Vector2 = .zero) {
      ...
      if health <= 0 {
        ...
        if Float.random(in: 0 ... 1) < healthDropChance {
          // Offset by half collectible size (8px) so it's centered on enemy
          let dropPos = deathPos - [4, 4]
          GameEvent.healthDropSpawned(position: dropPos).emit()
        }
      }
    }
