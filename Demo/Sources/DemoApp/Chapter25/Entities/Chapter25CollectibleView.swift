...
extension Chapter25 {
  struct CollectibleView: GView {
    ...
    var effectiveAnimation: SpriteAnimation<ItemSprite> {
      animation ?? SpriteAnimation(frames: [sprite], fps: 1)
    }

    var body: some GView {
      Area2D$ {
        AnimatedSpriteSheet(effectiveAnimation, position: [halfSize, halfSize])
        CollisionShape2D$()
