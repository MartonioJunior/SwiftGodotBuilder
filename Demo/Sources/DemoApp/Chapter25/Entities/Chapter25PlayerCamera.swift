...
extension Chapter25 {
  struct PlayerCamera: GView {
    ...
    let velocity: State<Vector2>
    let overlay: State<ActionOverlayState>
    let levelWidth: Float
    let levelHeight: Float
    let state: ObservableState<GameViewState>
    let config: PlayerConfig.Camera
    ...
    func update(_ camera: Camera2D, _ delta: Double) {
      ...
      let horizontalMovement = velocity.wrappedValue.x * Float(delta)

      if facing.wrappedValue == cameraFacing {
        ...
        if movementInDirection >= config.lookaheadThreshold {
          cameraFacing = facing.wrappedValue
          movementInDirection = 0
        }
      }

      // Track crouch time for camera look-down
      if overlay.wrappedValue.contains(.crouching) {
        crouchTimer += delta
      } else {
        crouchTimer = 0
      }
