...
extension Chapter25 {
  ...
  class GameProgress: Persistable {
    ...
    func isLevelUnlocked(_ levelId: String, levelIds: [String]) -> Bool {
      guard let currentIndex = levelIds.firstIndex(of: levelId) else { return false }
      if currentIndex == 0 { return true } // First level always unlocked
      let previousId = levelIds[currentIndex - 1]
      return getProgress(for: previousId).completed
    }
