...
extension Chapter16 {
  struct Player: GView {
    ...
    @State var playerScale = Vector2(x: 1, y: 1)
    ...
    func updatePlayer(_ player: CharacterBody2D, _ delta: Double) {
      ...
      if isDashing {
        ...
        if jumpBufferTimer > 0, canJump {
          ...
          Event.jumped(position: position + Vector2(x: size / 2, y: size)).emit()
        }
      ...
      if wasInAir, player.isOnFloor(), fallingVelocity > 100 {
        ...
        Event.landed(position: position + Vector2(x: size / 2, y: size), impact: Float(fallingVelocity)).emit()
      }
    ...
    func takeDamage(_ damage: Int) {
      ...
      if vm.playerHealth <= 0 {
        ...
        Event.playerDied(position: position + Vector2(x: size / 2, y: size / 2)).emit()
      } else {
        isInvincible = true
