...
struct Chapter9ParticleSpawner: GView {
  var body: some GView {
    Node2D$()
      .onEvent(Chapter9Event.self) { node, event in
        switch event {
  ...
  func spawnParticle(type: Chapter9ParticleType, at position: Vector2, parent: Node) {
    let config = getChapter9ParticleConfig(type: type)
  ...
  func getChapter9ParticleConfig(type: Chapter9ParticleType) -> ParticleConfig {
    switch type {
    case .jumpDust:
