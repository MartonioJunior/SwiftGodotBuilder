...
@Godot
final class Chapter5Game: Node2D {
  override func _ready() {
    let rootNode = Chapter5GameView().toNode()
    addChild(node: rootNode)
  }
}

struct Chapter5GameView: GView {
  ...
  // Game state
  @State var gameState: Chapter5GameState = .menu
  ...
  var body: some GView {
    Node2D$ {
      ...
      Chapter5Platform(x: 0, y: 165, width: screenWidth, height: platformHeight, color: .gray)

      // Starting area platforms
      Chapter5Platform(x: 20, y: 120, width: 80, height: platformHeight, color: .gray)
      Chapter5Platform(x: 120, y: 90, width: 100, height: platformHeight, color: .gray)

      // Mid-section platforms
      Chapter5Platform(x: 250, y: 120, width: 100, height: platformHeight, color: .gray)
      Chapter5Platform(x: 380, y: 80, width: 80, height: platformHeight, color: .gray)
      Chapter5Platform(x: 490, y: 110, width: 90, height: platformHeight, color: .gray)

      // Upper platforms
      Chapter5Platform(x: 600, y: 90, width: 100, height: platformHeight, color: .gray)

      // Goal area (green) - at the far right
      Chapter5Goal(x: 760, y: 145, size: 20)
    ...
    .onEvent(Chapter5Event.self) { _, event in
      ...
      case .enemyKilled:
        score += 10
        Chapter5Event.screenShake(intensity: 0.3).emit()
      case .playerHit:
        Chapter5Event.screenShake(intensity: 0.5).emit()
        Chapter5Event.screenFlash.emit()
