...
extension Chapter24 {
  struct BossView: GView {
    ...
    var bossColor: GState<Color> {
      $phase.computed(with: $isFlashing, $isDefeated) { phase, flashing, defeated in
    ...
    init(
      ...
      settings: ObservableState<GameSettings>,
    ...
    func updateBoss(_ delta: Double) {
      ...
      // Apply gravity
      if !isOnGround {
        velocity.y += gs.gravity * Float(delta)
      }
    ...
    func performShoot() {
      ...
      GameEvent.bossAttack(attackType: .shoot, position: projectilePos).emit()
    ...
    func performJump() {
      ...
      isOnGround = false
      GameEvent.bossAttack(attackType: .jump, position: position).emit()
    }
    ...
    func performCharge() {
      ...
      GameEvent.bossAttack(attackType: .charge, position: position).emit()
    }

    func performSummon() {
      currentAttack = .summon
      // Emit event for spawner to create minions
      GameEvent.bossAttack(attackType: .summon, position: position).emit()
    }
