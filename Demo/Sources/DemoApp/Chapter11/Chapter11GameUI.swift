...
struct Chapter11GameUI: GView {
  let state: ObservableState<Chapter11GameViewState>

  var body: some GView {
    CanvasLayer$ {
      Chapter11HUD(state: state)
      Chapter11MenuOverlay(state: state)
      Chapter11VictoryOverlay(state: state)
      Chapter11GameOverOverlay(state: state)
...
struct Chapter11HUD: GView {
  ...
  var body: some GView {
    ...
    VBoxContainer$ {
      // Title
      Label$()
        .text("CHAPTER 11: VISUAL FEEDBACK")
...
struct Chapter11MenuOverlay: GView {
  let state: ObservableState<Chapter11GameViewState>
    ...
    CenterContainer$ {
      PanelContainer$ {
        VBoxContainer$ {
          ...
          Label$()
            .text("• Squash & Stretch on Jump/Land\n• Color Changes for States\n• Rotation Effects for Death\n• Damage Flash & Enemy Knockback")
            .horizontalAlignment(.center)
            .theme(["fontColor": Color(r: 0.9, g: 0.9, b: 0.9)])
...
struct Chapter11VictoryOverlay: GView {
  let state: ObservableState<Chapter11GameViewState>
...
struct Chapter11GameOverOverlay: GView {
  let state: ObservableState<Chapter11GameViewState>
