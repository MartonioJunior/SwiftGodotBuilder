...
struct Chapter11ParticleSpawner: GView {
  var body: some GView {
    Node2D$()
      .onEvent(Chapter11Event.self) { node, event in
        switch event {
  ...
  func spawnParticle(type: Chapter11ParticleType, at position: Vector2, parent: Node) {
    let config = getChapter11ParticleConfig(type: type)
  ...
  func getChapter11ParticleConfig(type: Chapter11ParticleType) -> ParticleConfig {
    switch type {
    case .jumpDust:
    ...
    case .projectileTrail:
      return ParticleConfig(
        ...
        gravity: Vector2(x: 0, y: 0),
        color: Color(r: 1.0, g: 0.9, b: 0.3, a: 0.8)
      )
    }
  }
}
