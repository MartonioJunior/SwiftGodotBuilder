...
struct Chapter11Enemy: GView {
    ...
    Node2D$ {
      ...
      .onSignal(\.bodyEntered) { _, body in
        ...
        if !isDying, body is CharacterBody2D {
          Chapter11Event.playerHit(damage: touchDamage, position: position).emit()
        }
      }
  ...
  func takeDamage(_ damage: Int, from sourcePosition: Vector2 = .zero) {
    ...
    if health <= 0 {
      ...
      Chapter11Event.enemyKilled(position: position + Vector2(x: size / 2, y: size / 2)).emit()
    }
  }
