...
extension Chapter21 {
  struct Palette: Sendable {
    ...
    let npcPurple = Color(code: "#8844FF")
    let npcGreen = Color(code: "#44AA44")
    let npcBlue = Color(code: "#4488FF")
    let panelBgDialog = Color(r: 0.05, g: 0.05, b: 0.1, a: 0.95)

    var dialogPanelStyle: StyleBoxFlat$ {
      overlayPanel(bg: panelBgDialog, border: purple, borderWidth: 2, cornerRadius: 4, contentMargin: 8, expandMargin: 0)
    }
    ...
    var choiceButtonStyles: [String: StyleBoxFlat$] {
      [
        "normal": StyleBoxFlat$()
          ...
          .contentMargin(2),
        "hover": StyleBoxFlat$()
          .bgColor(purple.withAlpha(0.2))
          .borderColor(purple)
          .borderWidth(1)
          .cornerRadius(2)
          .contentMargin(2),
        "pressed": StyleBoxFlat$()
          .bgColor(purple.withAlpha(0.4))
          .borderColor(purple)
          .borderWidth(1)
          .cornerRadius(2)
          .contentMargin(2),
        "focus": StyleBoxFlat$()
          .bgColor(purple.withAlpha(0.3))
          .borderColor(white)
          .borderWidth(1)
          .cornerRadius(2)
          .contentMargin(2),
    ...
    func arrowShape(facingRight: Bool = true) -> PackedVector2Array {
      if facingRight {
    ...
    func diamondShape(size: Float) -> PackedVector2Array {
      let half = size / 2
      return PackedVector2Array([[0, -half], [half, 0], [0, half], [-half, 0]])
    }

    func heartShape(size: Float) -> PackedVector2Array {
      let s = size / 8
