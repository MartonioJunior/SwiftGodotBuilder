...
extension Chapter21 {
  ...
  class GameViewState {
    ...
    var isWelcome: Bool { gameState == .welcome }
    ...
    var playTimeDisplay: String { Chapter21.formatTime(playTime) }
    ...
    var currentMedal: Medal {
      guard let levelData = Chapter21.getLevelData(currentLevelId) else { return .none }
    ...
    var medalTargetsDisplay: String {
      guard let levelData = Chapter21.getLevelData(currentLevelId) else { return "" }
      return "🥇 \(Chapter21.formatTime(levelData.goldTime))  🥈 \(Chapter21.formatTime(levelData.silverTime))  🥉 \(Chapter21.formatTime(levelData.bronzeTime))"
    }

    var nextMedalTarget: String {
      guard let levelData = Chapter21.getLevelData(currentLevelId) else { return "" }
      if playTime <= levelData.goldTime { return "" }
      if playTime <= levelData.silverTime { return "🥇 -\(Chapter21.formatTime(playTime - levelData.goldTime))" }
      if playTime <= levelData.bronzeTime { return "🥈 -\(Chapter21.formatTime(playTime - levelData.silverTime))" }
      return "🥉 -\(Chapter21.formatTime(playTime - levelData.bronzeTime))"
    }

    var levelNameDisplay: String {
      if let levelData = Chapter21.getLevelData(currentLevelId) {
    ...
    func nextLevel() {
      ...
      if let levelData = Chapter21.getLevelData(nextId) {
