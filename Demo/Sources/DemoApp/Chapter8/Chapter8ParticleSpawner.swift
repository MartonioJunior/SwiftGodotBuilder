...
struct Chapter8ParticleSpawner: GView {
  var body: some GView {
    Node2D$()
      .onEvent(Chapter8Event.self) { node, event in
        switch event {
        ...
        case .goalReached, .gameReset, .attacked, .playerHit:
          break
        }
      }
  }

  func spawnParticle(type: Chapter8ParticleType, at position: Vector2, parent: Node) {
    let config = getChapter8ParticleConfig(type: type)
  ...
  func getChapter8ParticleConfig(type: Chapter8ParticleType) -> ParticleConfig {
    switch type {
    case .jumpDust:
