...
struct Chapter8Enemy: GView {
    ...
    Node2D$ {
      ...
      .onSignal(\.bodyEntered) { _, body in
        ...
        if !isDying, body is CharacterBody2D {
          Chapter8Event.playerHit(damage: touchDamage, position: position).emit()
        }
      }
  ...
  func takeDamage(_ damage: Int) {
    ...
    if health <= 0 {
      ...
      Chapter8Event.enemyKilled(position: position + Vector2(x: size / 2, y: size / 2)).emit()
    }
  }
