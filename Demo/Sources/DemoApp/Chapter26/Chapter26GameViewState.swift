...
extension Chapter26 {
  ...
  class GameViewState {
    ...
    // Collectibles (score-related, stay in game state)
    ...
    // Door positions (for level lookup)
    ...
    func prepareLevel(_ levelId: String, player: PlayerState) {
    ...
    func handlePlayerDied(player: PlayerState, currentScene: GameState) -> GameState? {
      guard currentScene == .playing else { return nil }
      deathCount += 1
      player.playerLives -= 1
      return player.playerLives > 0 ? .death : .gameOver
    }

    func respawnAfterDeath(player: PlayerState) {
      player.reset()
      GameEvent.gameReset.emit()
    }
    ...
    func handleCollected(_ item: Item, player: PlayerState) {
      switch item {
      case .coin:
        coinsCollected += 1
        score += 5
      case .key:
        ...
        score += 20
      case .ammo:
        player.handleAmmoCollected()
        score += 3
      case .health:
        player.handleHealthCollected()
      }
    }

    func handleCheckpointActivated(player: PlayerState, id: Int, position: Vector2) {
      guard player.handleCheckpointActivated(id: id, position: position) else { return }
      score += 25
    }
