...
extension Chapter19 {
  ...
  class GameViewState {
    ...
    var playTimeDisplay: String { Chapter19.formatTime(playTime) }
    ...
    var currentMedal: Medal {
      guard let levelData = Chapter19.getLevelData(currentLevelId) else { return .none }
    ...
    var medalTargetsDisplay: String {
      guard let levelData = Chapter19.getLevelData(currentLevelId) else { return "" }
      return "🥇 \(Chapter19.formatTime(levelData.goldTime))  🥈 \(Chapter19.formatTime(levelData.silverTime))  🥉 \(Chapter19.formatTime(levelData.bronzeTime))"
    }

    var nextMedalTarget: String {
      guard let levelData = Chapter19.getLevelData(currentLevelId) else { return "" }
      if playTime <= levelData.goldTime { return "" }
      if playTime <= levelData.silverTime { return "🥇 -\(Chapter19.formatTime(playTime - levelData.goldTime))" }
      if playTime <= levelData.bronzeTime { return "🥈 -\(Chapter19.formatTime(playTime - levelData.silverTime))" }
      return "🥉 -\(Chapter19.formatTime(playTime - levelData.bronzeTime))"
    }

    var levelNameDisplay: String {
      if let levelData = Chapter19.getLevelData(currentLevelId) {
    ...
    func nextLevel() {
      ...
      if let levelData = Chapter19.getLevelData(nextId) {
