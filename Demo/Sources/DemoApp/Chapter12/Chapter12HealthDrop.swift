import Foundation
import SwiftGodot
import SwiftGodotBuilder

struct Chapter12HealthDrop: GView {
  let spawnPosition: Vector2
  let despawnTime: Double = 10.0

  @State var position: Vector2
  @State var velocity: Vector2 = .init(x: 0, y: -50) // Float up initially
  @State var lifetime: Double = 0
  @State var collected: Bool = false

  init(spawnPosition: Vector2) {
    self.spawnPosition = spawnPosition
    position = spawnPosition
  }

  var body: some GView {
    CharacterBody2D$ {
      // Heart shape polygon
      Polygon2D$()
        .polygon(heartShape())
        .color(Color(r: 1.0, g: 0.3, b: 0.5))

      CollisionShape2D$()
        .shape(RectangleShape2D(w: 12, h: 12))
        .position([6, 6])

      // Detection area for player pickup
      Area2D$ {
        CollisionShape2D$()
          .shape(RectangleShape2D(w: 12, h: 12))
          .position([6, 6])
      }
      .collisionMask(.beta) // Player only
      .onSignal(\.bodyEntered) { node, body in
        guard !collected else { return }
        if body is CharacterBody2D {
          collected = true
          Chapter12Event.healthCollected(position: position).emit()
          node.getParent()?.queueFree()
        }
      }
    }
    .position($position)
    .velocity($velocity)
    .collisionLayer(.gamma)
    .collisionMask(.alpha) // Collide with terrain
    .onProcess { node, delta in
      guard !collected else { return }
      guard let body = node as? CharacterBody2D else { return }

      lifetime += delta

      // Despawn after timeout
      if lifetime >= despawnTime {
        node.queueFree()
        return
      }

      // Float up briefly, then fall
      if lifetime < 0.3 {
        velocity.y = -50
      } else {
        velocity.y += 300 * Float(delta) // Gravity
      }

      // Apply velocity and move
      body.velocity = velocity
      body.moveAndSlide()
      velocity = body.velocity
      position = body.position
    }
  }

  func heartShape() -> PackedVector2Array {
    // Heart shape centered around origin
    var points: [Vector2] = []
    // Top left curve
    points.append(Vector2(x: 0, y: 2))
    points.append(Vector2(x: -3, y: -1))
    points.append(Vector2(x: -5, y: 0))
    points.append(Vector2(x: -5, y: 2))
    points.append(Vector2(x: -4, y: 4))
    // Top middle
    points.append(Vector2(x: -2, y: 5))
    points.append(Vector2(x: 0, y: 5))
    points.append(Vector2(x: 2, y: 5))
    // Top right curve
    points.append(Vector2(x: 4, y: 4))
    points.append(Vector2(x: 5, y: 2))
    points.append(Vector2(x: 5, y: 0))
    points.append(Vector2(x: 3, y: -1))
    // Bottom point
    points.append(Vector2(x: 0, y: -6))
    // Finish shape
    points.append(Vector2(x: -3, y: -1))
    return PackedVector2Array(points)
  }
}
