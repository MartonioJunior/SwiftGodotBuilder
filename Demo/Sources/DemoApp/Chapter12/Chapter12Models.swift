...
enum Chapter12Event: EmittableEvent {
  // Game state events
  case goalReached
  case gameReset
  ...
  case weaponSwitched(weaponType: Chapter12WeaponType)
}

enum Chapter12ParticleType {
  case jumpDust
  case landingImpact
  case movementTrail
  case deathExplosion
...
enum Chapter12WeaponType {
  case melee
  case ranged
}

enum Chapter12EnemyType {
...
@Observable
class Chapter12GameViewState {
  var gameState: Chapter12GameState = .menu
  ...
  var maxAmmo: Int = 20
  var currentWeapon: Chapter12WeaponType = .melee
