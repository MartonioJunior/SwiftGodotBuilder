...
struct Chapter12EnemyProjectile: GView {
  ...
  var body: some GView {
    Area2D$ {
      ...
      Polygon2D$()
        ...
        .color(Color(r: 0.8, g: 0.2, b: 0.2)) // Red/orange for enemy projectiles
    ...
    .onSignal(\.bodyEntered) { node, body in
      ...
      if body is TileMap || body is StaticBody2D {
        Chapter12Event.projectileHitWall(position: position).emit()
      } else if body is CharacterBody2D {
        // Hit player
        Chapter12Event.playerHit(damage: 1, position: position).emit()
      }
      node.queueFree()
    }
