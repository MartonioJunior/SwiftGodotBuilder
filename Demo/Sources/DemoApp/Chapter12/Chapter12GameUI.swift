...
struct Chapter12GameUI: GView {
  let state: ObservableState<Chapter12GameViewState>

  var body: some GView {
    CanvasLayer$ {
      Chapter12HUD(state: state)
      Chapter12MenuOverlay(state: state)
      Chapter12VictoryOverlay(state: state)
      Chapter12GameOverOverlay(state: state)
...
struct Chapter12HUD: GView {
  ...
  var body: some GView {
    ...
    VBoxContainer$ {
      // Title
      Label$()
        .text("CHAPTER 12: ADVANCED ENEMIES")
...
struct Chapter12MenuOverlay: GView {
  let state: ObservableState<Chapter12GameViewState>
    ...
    CenterContainer$ {
      PanelContainer$ {
        VBoxContainer$ {
          ...
          Label$()
            .text("• Flying Enemies with Projectiles\n• Enemy Spawn Points with Respawn\n• Health Drops from Enemies\n• Dynamic Enemy Waves")
            .horizontalAlignment(.center)
            .theme(["fontColor": Color(r: 0.9, g: 0.9, b: 0.9)])
...
struct Chapter12VictoryOverlay: GView {
  let state: ObservableState<Chapter12GameViewState>
...
struct Chapter12GameOverOverlay: GView {
  let state: ObservableState<Chapter12GameViewState>
