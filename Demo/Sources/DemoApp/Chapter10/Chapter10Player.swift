...
struct Chapter10Player: GView {
  ...
  func updatePlayer(_ player: CharacterBody2D, _ delta: Double) {
    ...
    if isDashing {
      ...
      if jumpBufferTimer > 0, canJump {
        ...
        jumpBufferTimer = 0
        coyoteTimer = 0
        Chapter10Event.jumped(position: position + Vector2(x: size / 2, y: size)).emit()
      }
    ...
    if Action("switch_weapon").isJustPressed {
      ...
      Chapter10Event.weaponSwitched(weaponType: vm.currentWeapon).emit()
    }
    ...
    if Action("attack").isJustPressed, !isAttacking {
      if vm.currentWeapon == .melee {
        ...
        Chapter10Event.attacked(position: position).emit()
      } else {
        ...
        if vm.consumeAmmo() {
          ...
          Chapter10Event.projectileFired(position: projectilePos, direction: direction).emit()
        }
      }
    }
    ...
    if wasInAir, player.isOnFloor(), fallingVelocity > 100 {
      Chapter10Event.landed(position: position + Vector2(x: size / 2, y: size), impact: Float(fallingVelocity)).emit()
    }
  ...
  func takeDamage(_ damage: Int) {
    ...
    if vm.playerHealth <= 0 {
      vm.playerHealth = 0
      Chapter10Event.playerDied(position: position + Vector2(x: size / 2, y: size / 2)).emit()
    } else {
      isInvincible = true
