...
struct Chapter14Player: GView {
  ...
  private var vm: Chapter14GameViewState { state.wrappedValue }
  ...
  func updatePlayer(_ player: CharacterBody2D, _ delta: Double) {
    ...
    if isDashing {
      ...
      if jumpBufferTimer > 0, canJump {
        ...
        Chapter14Event.jumped(position: position + Vector2(x: size / 2, y: size)).emit()
      }
    ...
    if Action("switch_weapon").isJustPressed {
      ...
      Chapter14Event.weaponSwitched(weaponType: vm.currentWeapon).emit()
    }
    ...
    if Action("attack").isJustPressed, !isAttacking {
      if vm.currentWeapon == .melee {
        ...
        Chapter14Event.attacked(position: position).emit()
      } else {
        ...
        if vm.consumeAmmo() {
          ...
          Chapter14Event.projectileFired(position: projectilePos, direction: direction).emit()
        }
      }
    }
    ...
    if wasInAir, player.isOnFloor(), fallingVelocity > 100 {
      ...
      Chapter14Event.landed(position: position + Vector2(x: size / 2, y: size), impact: Float(fallingVelocity)).emit()
    }
  ...
  func takeDamage(_ damage: Int) {
    ...
    if vm.playerHealth <= 0 {
      ...
      Chapter14Event.playerDied(position: position + Vector2(x: size / 2, y: size / 2)).emit()
    } else {
      isInvincible = true
