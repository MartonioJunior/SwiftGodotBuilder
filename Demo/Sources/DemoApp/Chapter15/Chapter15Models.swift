...
enum Chapter15Event: EmittableEvent {
  // Game state events
  case goalReached
  case gameReset
  ...
  case weaponSwitched(weaponType: Chapter15WeaponType)
}

enum Chapter15ParticleType {
  case jumpDust
  case landingImpact
  case movementTrail
  case deathExplosion
...
enum Chapter15WeaponType {
  case melee
  case ranged
}

enum Chapter15EnemyType {
...
@Observable
class Chapter15GameSettings: Persistable {
  static let PersistenceKey = "chapter15_settings"
...
class GameProgress: Persistable {
  static let PersistenceKey = "chapter15_progress"
...
@Observable
class Chapter15GameViewState {
  var gameState: Chapter15GameState = .levelSelect
  ...
  var maxAmmo: Int = 20
  var currentWeapon: Chapter15WeaponType = .melee
  ...
  func nextLevel() {
    ...
    if let levelData = Chapter15.getLevelData(nextId) {
  ...
  func restartLevel() {
    if let levelData = Chapter15.getLevelData(currentLevelId) {
