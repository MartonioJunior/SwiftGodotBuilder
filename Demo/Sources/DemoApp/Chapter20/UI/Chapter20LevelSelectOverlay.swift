      ...
      CenterContainer$ {
        PanelContainer$ {
          VBoxContainer$ {
            ...
            Label$()
              .text("• Interactive NPCs\n• Dialog system\n• Press E to interact\n• Story conversations")
              .horizontalAlignment(.center)
              .theme(["fontColor": palette.lightGray])

            Control$().minSize([0, 4])

            LevelRow(levelData: Chapter20.levels[0], state: state, progress: progress, isFirst: true, firstButtonRef: $firstButton)
            LevelRow(levelData: Chapter20.levels[1], state: state, progress: progress)
            LevelRow(levelData: Chapter20.levels[2], state: state, progress: progress)
            LevelRow(levelData: Chapter20.levels[3], state: state, progress: progress)
