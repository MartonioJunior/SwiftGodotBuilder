...
@Godot
final class Chapter20Game: Node2D {
  override func _ready() {
    let rootNode = Chapter20.GameView().toNode()
...
extension Chapter20 {
  struct GameView: GView {
    ...
    var playerSpawnPoint: Vector2 {
      Chapter20.getLevelData(state.currentLevelId)?.playerSpawnPoint ?? [40, 100]
    }
    ...
    var body: some GView {
      ...
      .onEvent(DialogEvent.self) { _, event in
        switch event {
        case let .started(npcId):
          handleDialogStarted(npcId: npcId)
        default:
          break
        }
      }
...
extension Chapter20.GameView {
  ...
  func handleDialogStarted(npcId: String) {
    // Get visit count from game state
    let visitCount = state.beginDialogVisit(npcId: npcId)
    let dialogState = DialogState(visitCount: visitCount)

    // Create dialog using factory function
    guard let dialog = Chapter20.makeDialog(
      for: npcId,
      state: dialogState,
      gameState: state,
      progress: progress
