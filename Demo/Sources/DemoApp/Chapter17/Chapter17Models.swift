...
enum Chapter17 {
  ...
  class GameSettings: Persistable {
    static let PersistenceKey = "chapter17_settings"
  ...
  class GameProgress: Persistable {
    static let PersistenceKey = "chapter17_progress"
  ...
  class GameViewState {
    ...
    var deathsDisplay: String {
      "Deaths: \(deathCount)"
    }

    var levelNameDisplay: String {
      if let levelData = Chapter17.getLevelData(currentLevelId) {
    ...
    func nextLevel() {
      ...
      if let levelData = Chapter17.getLevelData(nextId) {
