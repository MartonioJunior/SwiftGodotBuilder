...
@Godot
final class Chapter17Game: Node2D {
  override func _ready() {
    let rootNode = Chapter17.GameView().toNode()
...
extension Chapter17 {
  struct GameView: GView {
    ...
    var playerSpawnPoint: Vector2 {
      Chapter17.getLevelData(state.currentLevelId)?.playerSpawnPoint ?? [40, 100]
    }
  ...
  func handleInput() {
    ...
    if Action("start").isJustPressed {
      ...
      case .levelSelect, .gameOver, .levelComplete:
        if let levelData = Chapter17.getLevelData(state.currentLevelId) {
