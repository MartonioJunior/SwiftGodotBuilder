...
extension Chapter22 {
  struct LeaderboardOverlay: GView {
    ...
    func updateLeaderboard() {
      ...
      levelName = Chapter22.getLevelData(levelId)?.name ?? "Level \(levelId)"
            ...
            HBoxContainer$ {
              Button$()
                ...
                .onSignal(\.pressed) { [state] _ in
                  ...
                  let newId = currentId > 1 ? currentId - 1 : Chapter22.levels.count
              ...
              Button$()
                ...
                .onSignal(\.pressed) { [state] _ in
                  let currentId = state.wrappedValue.leaderboardLevelId
                  let newId = currentId < Chapter22.levels.count ? currentId + 1 : 1
                  state.wrappedValue.showLeaderboard(for: newId)
                }
            }
            .theme(["separation": 8])
