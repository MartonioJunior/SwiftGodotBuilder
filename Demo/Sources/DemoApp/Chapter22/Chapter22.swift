...
@Godot
final class Chapter22Game: Node2D {
  override func _ready() {
    let rootNode = Chapter22.GameView().toNode()
...
extension Chapter22 {
  struct GameView: GView {
    ...
    var currentLevelData: LevelData? {
      Chapter22.getLevelData(state.currentLevelId)
    }
    ...
    var body: some GView {
      Node2D$ {
        ...
        .watch($state, \.gameState) { node, gameState in
          // Show game content only when in-game (playing, paused, levelComplete, gameOver, dialog)
          let inGame = gameState == .playing || gameState == .paused || gameState == .levelComplete || gameState == .gameOver || gameState == .dialog
          node.visible = inGame
        }

        GameUI(state: $state, settings: $settings, progress: $progress, transitionState: $transitionState)
...
extension Chapter22.GameView {
  ...
  func handlePlayerDeath(at position: Vector2) {
    // Convert world position to normalized screen coordinates (0-1)
    // The player position is in level coordinates, we need to estimate screen center
    // For simplicity, use a position based on where the player died relative to viewport
    let normalizedX = min(1, max(0, position.x / viewportWidth))
    let normalizedY = min(1, max(0, position.y / viewportHeight))

    // Check if player will have lives remaining
    let willHaveLivesRemaining = state.playerLives > 1

    if willHaveLivesRemaining {
      // Iris transition centered on death position, then respawn
      ...
      // No lives remaining - just show game over (no transition, the overlay handles it)
      state.handlePlayerDied()
    }
  }
  ...
  func handleDialogStarted(
    npcId: String,
    makeDialog: (DialogState, Chapter22.GameViewState, Chapter22.GameProgress) -> DialogDefinition?
  ) {
