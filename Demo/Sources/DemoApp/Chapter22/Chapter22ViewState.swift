...
extension Chapter22 {
  ...
  class GameViewState {
    ...
    var playTimeDisplay: String { Chapter22.formatTime(playTime) }
    ...
    var currentMedal: Medal {
      guard let levelData = Chapter22.getLevelData(currentLevelId) else { return .none }
    ...
    var medalTargetsDisplay: String {
      guard let levelData = Chapter22.getLevelData(currentLevelId) else { return "" }
      return "🥇 \(Chapter22.formatTime(levelData.goldTime))  🥈 \(Chapter22.formatTime(levelData.silverTime))  🥉 \(Chapter22.formatTime(levelData.bronzeTime))"
    }

    var nextMedalTarget: String {
      guard let levelData = Chapter22.getLevelData(currentLevelId) else { return "" }
      if playTime <= levelData.goldTime { return "" }
      if playTime <= levelData.silverTime { return "🥇 -\(Chapter22.formatTime(playTime - levelData.goldTime))" }
      if playTime <= levelData.bronzeTime { return "🥈 -\(Chapter22.formatTime(playTime - levelData.silverTime))" }
      return "🥉 -\(Chapter22.formatTime(playTime - levelData.bronzeTime))"
    }

    var levelNameDisplay: String {
      if let levelData = Chapter22.getLevelData(currentLevelId) {
    ...
    func nextLevel() {
      ...
      if let levelData = Chapter22.getLevelData(nextId) {
