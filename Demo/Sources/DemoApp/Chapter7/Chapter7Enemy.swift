...
struct Chapter7Enemy: GView {
    ...
    Node2D$ {
      ...
      .onSignal(\.bodyEntered) { _, body in
        ...
        if !isDying, body is CharacterBody2D {
          Chapter7Event.playerHit(damage: touchDamage, position: position).emit()
        }
      }
  ...
  func takeDamage(_ damage: Int) {
    ...
    if health <= 0 {
      ...
      Chapter7Event.enemyKilled(position: position + Vector2(x: size / 2, y: size / 2)).emit()
    }
  }
