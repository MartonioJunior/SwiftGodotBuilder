...
@Godot
final class Chapter7Game: Node2D {
  override func _ready() {
    let rootNode = Chapter7GameView().toNode()
    addChild(node: rootNode)
  }
}

struct Chapter7GameView: GView {
  let screenWidth: Float = 800
  let screenHeight: Float = 180
  let platformHeight: Float = 8
  let gravity: Float = 400

  @ObservableState var state = Chapter7GameViewState()

  var body: some GView {
    Node2D$ {
      // Ground platform - full width
      Chapter7Platform(x: 0, y: 165, width: screenWidth, height: platformHeight, color: .gray)

      // Starting area platforms
      Chapter7Platform(x: 20, y: 120, width: 80, height: platformHeight, color: .gray)
      Chapter7Platform(x: 120, y: 90, width: 100, height: platformHeight, color: .gray)

      // Mid-section platforms
      Chapter7Platform(x: 250, y: 120, width: 100, height: platformHeight, color: .gray)
      Chapter7Platform(x: 380, y: 80, width: 80, height: platformHeight, color: .gray)
      Chapter7Platform(x: 490, y: 110, width: 90, height: platformHeight, color: .gray)

      // Upper platforms
      Chapter7Platform(x: 600, y: 90, width: 100, height: platformHeight, color: .gray)

      // Goal area (green) - at the far right
      Chapter7Goal(x: 760, y: 145, size: 20)
      ...
      // UI Overlay
      Chapter7GameUI(state: $state)

      // Particle spawner
      Chapter7ParticleSpawner()

      // Audio manager
      Chapter7AudioManager(state: $state)
    }
    .onEvent(Chapter7Event.self) { _, event in
      ...
      case .playerHit:
        ...
        applyScreenFlash()
      case .gameReset, .jumped, .landed, .attacked, .coinCollected:
        break
      }
    }
    .onReady { _ in
