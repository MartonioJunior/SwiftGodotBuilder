...
struct Chapter7ParticleSpawner: GView {
  var body: some GView {
    Node2D$()
      .onEvent(Chapter7Event.self) { node, event in
        switch event {
  ...
  func spawnParticle(type: Chapter7ParticleType, at position: Vector2, parent: Node) {
    let config = getChapter7ParticleConfig(type: type)
  ...
  func getChapter7ParticleConfig(type: Chapter7ParticleType) -> ParticleConfig {
    switch type {
    case .jumpDust:
