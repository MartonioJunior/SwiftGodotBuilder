...
struct Chapter6ParticleSpawner: GView {
  var body: some GView {
    Node2D$()
      .onEvent(Chapter6Event.self) { node, event in
  ...
  func spawnParticle(type: Chapter6ParticleType, at position: Vector2, parent: Node) {
    let config = getChapter6ParticleConfig(type: type)
  ...
  func getChapter6ParticleConfig(type: Chapter6ParticleType) -> ParticleConfig {
    switch type {
    case .jumpDust:
    ...
    case .movementTrail:
      // Custom config for Chapter 6's blue player trail
      return ParticleConfig(
        amount: 3,
        lifetime: 0.2,
        explosiveness: 0.0,
