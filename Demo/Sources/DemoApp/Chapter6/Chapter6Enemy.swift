...
struct Chapter6Enemy: GView {
    ...
    Node2D$ {
      ...
      .onSignal(\.bodyEntered) { _, body in
        ...
        if !isDying, body is CharacterBody2D {
          Chapter6Event.playerHit(damage: touchDamage).emit()
        }
      }
  ...
  func takeDamage(_ damage: Int) {
    health -= damage
    // Hit particles
    Chapter6Event.spawnParticles(type: .enemyHit, position: position + Vector2(x: size / 2, y: size / 2)).emit()

    if health <= 0 {
      health = 0
      isDying = true
      deathTimer = deathFadeDuration
      Chapter6Event.enemyKilled.emit()
    }
  }
