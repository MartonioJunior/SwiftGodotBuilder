...
struct Chapter6Player: GView {
  let spawnPoint: Vector2
  let screenWidth: Float
  let screenHeight: Float
  let gravity: Float
  let state: ObservableState<Chapter6GameViewState>

  private var vm: Chapter6GameViewState { state.wrappedValue }
  ...
  init(
    ...
    screenHeight: Float,
    gravity: Float,
    state: ObservableState<Chapter6GameViewState>
  ) {
  ...
  func updatePlayer(_ player: CharacterBody2D, _ delta: Double) {
    ...
    if Action("jump").isJustPressed, player.isOnFloor() {
      vel.y = -jumpSpeed
      // Jump dust particles
      Chapter6Event.spawnParticles(type: .jumpDust, position: position + Vector2(x: size / 2, y: size)).emit()
    }
    ...
    if wasInAir, player.isOnFloor(), fallingVelocity > 100 {
      Chapter6Event.spawnParticles(type: .landingImpact, position: position + Vector2(x: size / 2, y: size)).emit()
    }
    ...
    if abs(velocity.x) > 50, player.isOnFloor() {
      ...
      if movementTimer > 0.1 {
        Chapter6Event.spawnParticles(type: .movementTrail, position: position + Vector2(x: size / 2, y: size / 2)).emit()
        movementTimer = 0
      }
    }
  ...
  func takeDamage(_ damage: Int) {
    ...
    if vm.playerHealth <= 0 {
      ...
      Chapter6Event.spawnParticles(type: .deathExplosion, position: position + Vector2(x: size / 2, y: size / 2)).emit()
      Chapter6Event.playerDied.emit()
    } else {
      isInvincible = true
