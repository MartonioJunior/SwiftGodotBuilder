...
enum Chapter13Event: EmittableEvent {
  // Game state events
  case goalReached
  case gameReset
  ...
  case weaponSwitched(weaponType: Chapter13WeaponType)
}

enum Chapter13ParticleType {
  case jumpDust
  case landingImpact
  case movementTrail
  case deathExplosion
...
enum Chapter13WeaponType {
  case melee
  case ranged
}

enum Chapter13EnemyType {
...
@Observable
class Chapter13GameViewState {
  var gameState: Chapter13GameState = .menu
  ...
  var maxAmmo: Int = 20
  var currentWeapon: Chapter13WeaponType = .melee
