...
struct Chapter13Player: GView {
  ...
  private var vm: Chapter13GameViewState { state.wrappedValue }
  ...
  init(
    ...
    screenHeight: Float,
    gravity: Float,
    state: ObservableState<Chapter13GameViewState>
  ) {
  ...
  func updatePlayer(_ player: CharacterBody2D, _ delta: Double) {
    ...
    if isDashing {
      ...
      if jumpBufferTimer > 0, canJump {
        ...
        Chapter13Event.jumped(position: position + Vector2(x: size / 2, y: size)).emit()
      }
    ...
    if Action("switch_weapon").isJustPressed {
      ...
      Chapter13Event.weaponSwitched(weaponType: vm.currentWeapon).emit()
    }
    ...
    if Action("attack").isJustPressed, !isAttacking {
      if vm.currentWeapon == .melee {
        ...
        Chapter13Event.attacked(position: position).emit()
      } else {
        ...
        if vm.consumeAmmo() {
          ...
          Chapter13Event.projectileFired(position: projectilePos, direction: direction).emit()
        }
      }
    }
    ...
    if wasInAir, player.isOnFloor(), fallingVelocity > 100 {
      ...
      Chapter13Event.landed(position: position + Vector2(x: size / 2, y: size), impact: Float(fallingVelocity)).emit()
    }
  ...
  func takeDamage(_ damage: Int) {
    ...
    if vm.playerHealth <= 0 {
      ...
      Chapter13Event.playerDied(position: position + Vector2(x: size / 2, y: size / 2)).emit()
    } else {
      isInvincible = true
