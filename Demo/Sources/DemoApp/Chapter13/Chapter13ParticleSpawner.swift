...
struct Chapter13ParticleSpawner: GView {
  var body: some GView {
    Node2D$()
  ...
  func spawnParticle(type: Chapter13ParticleType, at position: Vector2, parent: Node) {
    let palette = Palette()
    let config = getChapter13ParticleConfig(type: type, palette: palette)
  ...
  func getChapter13ParticleConfig(type: Chapter13ParticleType, palette: Palette) -> ParticleConfig {
    switch type {
    case .jumpDust:
