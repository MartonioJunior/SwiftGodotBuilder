...
struct Chapter13Enemy: GView {
  let type: Chapter13EnemyType
  ...
  init(
    type: Chapter13EnemyType = .patrol,
    spawnPoint: Vector2,
    patrolLeft: Float,
    patrolRight: Float,
    gravity: Float,
    ...
    Node2D$ {
      ...
      .onSignal(\.bodyEntered) { _, body in
        ...
        if !isDying, body is CharacterBody2D {
          Chapter13Event.playerHit(damage: touchDamage, position: position).emit()
        }
      }
    ...
    if knockbackTimer > 0 {
      ...
      if type == .flyer {
        ...
        if shootTimer <= 0 {
          ...
          Chapter13Event.enemyProjectileFired(
  ...
  func takeDamage(_ damage: Int, from sourcePosition: Vector2 = .zero) {
    ...
    if health <= 0 {
      ...
      Chapter13Event.enemyKilled(position: deathPos).emit()

      // 25% chance to drop health
      if Float.random(in: 0 ... 1) < healthDropChance {
        Chapter13Event.healthDropSpawned(position: deathPos).emit()
      }
    }
  }
